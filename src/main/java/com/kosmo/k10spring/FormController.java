...
/*
사용자의 요청을 제일먼저 받는 DispacherServlet은 기본
패키지로 지정된 com.kosmo.k10spring을 스캔하여 컨트롤러 클래스를
찾는다. 그리고 해당 요청명에 매핑되는 메소드를
찾아 요청을 전달하고 비즈니스로직을 수행한다.

@Controller
	: 해당 클래스를 컨트롤러로 사용하고 싶을때 클래스명앞에 선언한다.
	패캐지를 스캔할때 해당 어노테이션이 있는 클래스를 찾아서 요청한다.
...
public class FormController {
	...
	public String loginMember(HttpServletRequest req, Model model) {
		//톰켓서버위에서 jsp기반으로 만드는 영역이므로 req도 당연히 쓸수있다! model객체만 추가된 부분
		...
		model.addAttribute("message", "로그인정보가 전달되었습니다.");
		/*
